        ...
        <div className="container-custom">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 sm:gap-10 items-start">
            ...
            <div className="md:col-span-6 flex flex-col gap-4 sm:gap-6 md:sticky md:top-24 self-start z-10">
              <div className="relative rounded-2xl sm:rounded-3xl overflow-hidden border-2 border-[#cfa144]/80 bg-[#050505] p-5 sm:p-8 md:p-12 flex items-center justify-center min-h-[260px] sm:min-h-[380px] md:min-h-[460px] shadow-2xl">
                <img
                  src={product.image}
                  alt={product.name}
                ...
                />
                <span className="absolute top-3 right-3 sm:top-4 sm:right-4 bg-[#cfa144] text-[#1A1D20] text-[10px] sm:text-xs font-bold px-2.5 sm:px-3.5 py-1 sm:py-1.5 rounded-full uppercase tracking-wider shadow">
                  {product.capacity}
                </span>
              ...
              <div className="bg-white p-5 sm:p-8 rounded-2xl sm:rounded-3xl border border-[#E6DBC6] shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                  <div className="bg-[#FAF8F4] p-3.5 rounded-2xl border border-[#E6DBC6] flex items-center gap-3">
                    <ShieldCheck className="w-5 h-5 text-[#a8812f] shrink-0" />
                    <div>
                  ...
                  <div className="bg-[#FAF8F4] p-3.5 rounded-2xl border border-[#E6DBC6] flex items-center gap-3">
                    <Flame className="w-5 h-5 text-[#a8812f] shrink-0" />
                    <div>
                    ...
                    <table className="w-full min-w-[420px] text-left border-collapse">
                      <tbody>
                        <tr className="border-b border-[#E6DBC6] bg-[#FAF8F4]">
                          ...
                          <td className="p-3 font-bold text-[#a8812f]">{product.capacity}</td>
                        </tr>
                    ...
                    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                      {product.features.map((feature, idx) => (
                        <li key={idx} className="flex items-start gap-2.5 bg-[#FAF8F4] border border-[#E6DBC6] rounded-2xl px-3.5 py-3 text-xs text-gray-700">
                          <CheckCircle2 className="w-4 h-4 text-[#a8812f] shrink-0 mt-0.5" />
                    ...
                    <div className="flex flex-wrap gap-2">
                      {product.applications.map((app, idx) => (
                        <span key={idx} className="inline-flex items-center gap-1.5 text-xs font-semibold bg-[#FAF8F4] text-gray-700 px-3 py-1 rounded-full border border-[#E6DBC6]">
                          <CheckCircle2 className="w-3.5 h-3.5 text-[#a8812f]" /> {app}
                        </span>
                      ))}
                    </div>
                  </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <Link href={`/downloads#${category.slug}`} className="inline-flex items-center gap-2 text-xs font-bold text-[#1A1D20] bg-[#FAF8F4] border border-[#E6DBC6] rounded-xl px-4 py-2.5 hover:border-[#cfa144] transition-colors">
                    <FileDown className="w-4 h-4 text-[#a8812f]" /> Datasheet &amp; brochure
                  </Link>
                  <span className="text-[11px] text-gray-500">Specifications, CAD drawings and certifications for the {category.name} line.</span>
                </div>
              </div>

              <div className="bg-white p-5 sm:p-8 rounded-2xl sm:rounded-3xl border-2 border-[#cfa144]/70 shadow-md">
                ...
                <form className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="tel" placeholder="+91 98000 00000" className="w-full bg-[#FAF8F4] border border-[#E6DBC6] rounded-xl px-4 py-3 text-xs focus:outline-none focus:border-[#cfa144]" />
                    </div>
                    <div>
                      <label className="text-[11px] font-bold text-[#1A1D20] uppercase block mb-1">Estimated Quantity</label>
                      <input type="text" placeholder="e.g. 5,000 Pcs" className="w-full bg-[#FAF8F4] border border-[#E6DBC6] rounded-xl px-4 py-3 text-xs focus:outline-none focus:border-[#cfa144]" />
                    </div>
                  </div>

                  <div>
                    <label className="text-[11px] font-bold text-[#1A1D20] uppercase block mb-1">Inquiry Message *</label>
                    <textarea rows={3} placeholder={`Inquiry for ${product.name} (${product.capacity})...`} required className="w-full bg-[#FAF8F4] border border-[#E6DBC6] rounded-xl p-4 text-xs focus:outline-none focus:border-[#cfa144]"></textarea>
                  </div>

                  <button type="submit" className="w-full bg-[#cfa144] hover:bg-[#1A1D20] hover:text-[#faf8f4] text-[#1A1D20] font-bold py-3.5 rounded-xl text-xs uppercase tracking-wider shadow-md transition-all flex items-center justify-center gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {relatedCategoryProducts.map((rel) => (
                  ...
                  >
                    ...
                    <div>
                      <span className="text-[10px] font-bold text-[#a8812f] uppercase block">{rel.capacity}</span>
                      <h4 className="text-sm font-bold text-[#1A1D20] group-hover:text-[#a8812f] transition-colors">{rel.name}</h4>
                    </div>
                  </Link>
                ))}
              </div>
