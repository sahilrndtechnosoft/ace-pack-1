...
function DownloadRow({ item }: { item: DownloadItem }) {
  ...
  return (
    <div id={item.id} className="scroll-mt-28 bg-white p-5 sm:p-6 rounded-3xl border border-[#E6DBC6] hover:border-[#b89858] transition-colors flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
      <div className="w-11 h-11 rounded-2xl bg-[#b89858]/15 text-[#b89858] flex items-center justify-center shrink-0 border border-[#b89858]/30">
        <Icon className="w-5 h-5" />
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-2 mb-1">
          <h3 className="text-sm sm:text-base font-bold text-[#1A1D20]">{item.title}</h3>
          <span className="text-[10px] font-bold uppercase tracking-wider text-[#b89858] bg-[#FAF8F4] border border-[#E6DBC6] rounded-full px-2 py-0.5">{item.type}</span>
        </div>
        <p className="text-xs text-gray-600 leading-relaxed">{item.description}</p>
      </div>
      {live ? (
        <a href={item.file} download className="inline-flex items-center justify-center gap-2 shrink-0 bg-[#b89858] hover:bg-[#9e8042] text-white font-bold py-3 px-5 rounded-xl text-xs uppercase tracking-wider transition-all">
          <FileDown className="w-4 h-4" /> Download PDF
        </a>
      ) : (
        <a href={requestHref(item)} className="inline-flex items-center justify-center gap-2 shrink-0 bg-[#FAF8F4] hover:bg-white border border-[#E6DBC6] hover:border-[#b89858] text-[#1A1D20] font-bold py-3 px-5 rounded-xl text-xs uppercase tracking-wider transition-all">
          <Mail className="w-4 h-4 text-[#b89858]" /> Request by email
        </a>
      )}
    </div>
  );
        ...
        <div className="container-custom">
          <nav aria-label="Download sections" className="flex flex-wrap gap-2 mb-10">
            {downloadGroups.map((group) => (
              <a key={group.id} href={`#${group.id}`} className="text-xs font-bold text-[#1A1D20] bg-white border border-[#E6DBC6] hover:border-[#b89858] rounded-full px-4 py-2 transition-colors">
                {group.title}
              </a>
            ))}
          </nav>
          ...
          <div className="mt-14 bg-[#1A1D20] text-white p-8 sm:p-10 rounded-3xl shadow-lg flex flex-col md:flex-row md:items-center gap-6">
            ...
            <Link href="/contact" className="inline-flex items-center justify-center gap-2 shrink-0 bg-[#b89858] hover:bg-[#9e8042] text-white font-bold py-3.5 px-7 rounded-xl text-xs uppercase tracking-wider transition-all">
