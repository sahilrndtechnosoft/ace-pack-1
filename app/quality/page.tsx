  ...
  return (
    <div className="bg-[#FAF8F4] min-h-screen text-[#1A1D20] pb-24">
      ...
      <section className="py-12 md:py-16">
        <div className="container-custom">
          <div className="max-w-2xl mb-10">
            <span className="text-[10px] font-bold text-[#a8812f] uppercase tracking-wider block mb-2">Quality Standards</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {certifications.map((cert, idx) => (
              <Reveal key={cert.id} type="fade-up" delay={idx * 0.08}>
                <div className="group bg-white p-6 rounded-3xl border border-[#E6DBC6] hover:border-[#cfa144] hover:shadow-md hover:-translate-y-1 transition-all duration-300 h-full flex flex-col">
                  <div className="w-12 h-12 rounded-2xl bg-[#cfa144]/15 text-[#a8812f] flex items-center justify-center border border-[#cfa144]/30 mb-4 group-hover:scale-110 transition-transform duration-300">
                    <ShieldCheck className="w-6 h-6" />
                  </div>
                  <span className="text-[10px] font-bold text-[#a8812f] uppercase tracking-wider mb-1">{cert.scope}</span>
      ...
      <section className="py-12 md:py-16 bg-white border-y border-[#E6DBC6]">
        <div className="container-custom">
          <div className="max-w-2xl mb-10">
            <span className="text-[10px] font-bold text-[#a8812f] uppercase tracking-wider block mb-2">Inspection &amp; Testing</span>
            ...
            {qualityChecks.map((check, idx) => (
              <Reveal key={check.step} type="fade-up" delay={idx * 0.06}>
                <div className="bg-[#FAF8F4] p-6 sm:p-7 rounded-3xl border border-[#E6DBC6] flex flex-col sm:flex-row sm:items-start gap-4 sm:gap-6">
                  <span className="text-3xl font-extrabold text-[#a8812f] shrink-0 leading-none">{check.step}</span>
                  ...
                  <span className="inline-flex items-center gap-1.5 self-start text-[11px] font-bold text-[#1A1D20] bg-white border border-[#E6DBC6] rounded-full px-3 py-1.5 whitespace-nowrap">
                    <ClipboardCheck className="w-3.5 h-3.5 text-[#a8812f]" /> {check.standard}
                  </span>
                </div>
              </Reveal>
            ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              <ul className="space-y-3">
                {complianceStatements.map((statement) => (
                  <li key={statement} className="flex items-start gap-3 text-xs sm:text-sm text-gray-700">
                    <CheckCircle2 className="w-[18px] h-[18px] text-[#a8812f] shrink-0 mt-0.5" />
                    <span>{statement}</span>
                  </li>
                ))}
              </ul>
            ...
            <div className="lg:col-span-5 bg-[#1A1D20] text-white p-8 rounded-3xl shadow-lg">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <Link href="/downloads#certifications" className="inline-flex items-center justify-center gap-2 bg-[#cfa144] hover:bg-[#1A1D20] hover:text-[#faf8f4] text-[#1A1D20] font-bold py-3.5 px-6 rounded-xl text-xs uppercase tracking-wider transition-all">
