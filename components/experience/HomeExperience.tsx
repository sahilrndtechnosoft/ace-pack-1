...
import { products, initialSceneState } from './config';
...
function Count({ value, suffix='' }: { value:number; suffix?:string }) {
  ...
  useEffect(()=>{
    ...
    const context=gsap.context(()=>{const counter={value:0};gsap.to(counter,{value,duration:1.6,ease:'power2.out',scrollTrigger:{trigger:el,start:'top 90%',once:true},onUpdate:()=>{el.textContent=Math.round(counter.value).toLocaleString('en-IN')+suffix;}});});return()=>context.revert();
  },[value,suffix,reduced]);
  return <span ref={ref}>{value.toLocaleString('en-IN')}{suffix}</span>;
}
const craftFacts=[
  {title:'Every curve.\nConsidered.',body:'A form that feels right in the hand. Rounded corners, a sturdy base, and just the right proportions.',tag:'01 / Precision in the details'},
  {title:'Open up\nto better.',body:'One continuous hinge. A carefully formed rim. Scroll to see how every part works together.',tag:'02 / A closer look inside'},
  {title:'Ready for\nthe real world.',body:'From the first prep to the last delivery. Practical packaging designed around the way you serve.',tag:'03 / Purpose in every pack'},
];
export default function HomeExperience() {
  ...
  const [retry,setRetry]=useState(0);const dragging=useRef<number|null>(null);const reduced=useReducedMotion();
  ...
  useEffect(()=>{gsap.to(state.current,{inspect:openLid?1:0,duration:.8,ease:'power2.inOut'});},[openLid]);
  useEffect(()=>{
    if(mode!=='3d')return;
  ...
  return <MotionConfig reducedMotion="user"><div ref={root} className={`xp-home ${live?'xp-live':'xp-static'}`} data-render-reason={fallbackReason}>
    ...
    <section id="main-content" className="xp-hero">
      <div className="xp-hero-topline"><span><i/> Designed around the way you serve.</span><span>AcePack / Container solutions</span></div>
      <div className="xp-hero-copy"><p className="xp-eyebrow">For the good inside.</p><h1 aria-label="Beyond the box.">{['Beyond','the box.'].map((line,i)=><span className={`xp-title-mask ${i?'xp-serif':''}`} key={line}><motion.span initial={reduced?false:{y:'110%'}} animate={{y:ready||!live?0:'110%'}} transition={{duration:1.1,delay:.15*i,ease:[.22,1,.36,1]}}>{line}</motion.span></span>)}</h1><p className="xp-hero-description">Great food deserves a great arrival.<br/>Thoughtfully made packaging. Endless possibilities.</p><MagneticLink href="#craft">Unpack the difference</MagneticLink><div className="xp-hero-material"><span className="xp-material-swatch"/><span>Good form. Better function.<br/><b>Made for your everyday.</b></span></div></div>
      ...
      {live&&<div className="xp-model-interaction" onPointerDown={e=>{dragging.current=e.clientX;e.currentTarget.setPointerCapture(e.pointerId);}} onPointerMove={e=>{if(dragging.current!==null){state.current.drag+=(e.clientX-dragging.current)*.008;dragging.current=e.clientX;}}} onPointerUp={()=>{dragging.current=null;}} onPointerCancel={()=>{dragging.current=null;}} aria-hidden="true"/>}
      <div className="xp-orbit" aria-hidden="true"><span/><span/></div>
      <div className="xp-hero-caption"><span className="xp-eyebrow">01 / The everyday essential</span><span className="xp-caption-rule"/><span>Precision. In every dimension.</span></div>
    ...
    <section id="collection" className="xp-collection">{live&&products.map((p,i)=><span key={p.id} className="xp-range-anchor" id={`product-${p.id}`} style={{top:`${i*100}vh`}}/>)}<div className="xp-collection-stage"><div className="xp-collection-heading"><div><span className="xp-eyebrow">02 / Made for every menu</span><h2 data-split>Find your <em>form.</em></h2></div><Link href="/categories">View all categories <ArrowUpRight size={18}/></Link></div><div className="xp-product-panels">{products.map((p,i)=><motion.article key={`${p.id}-${revealMotion}`} className="xp-product" aria-hidden={live&&activeProduct!==i} inert={live&&activeProduct!==i} {...panelMotion(!live||activeProduct===i,35)} transition={{duration:.6,ease:[.22,1,.36,1]}}><div className="xp-product-copy"><span className="xp-eyebrow">{p.number} / {p.title}</span><h3>{p.name.split('\n').map((line,j)=><span key={line} className={j?'xp-serif':''}>{line}</span>)}</h3><p>{p.description}</p><dl><div><dt>Material</dt><dd>{p.material}</dd></div><div><dt>Format</dt><dd>{p.capacity}</dd></div><div><dt>Made for</dt><dd>{p.use}</dd></div></dl><MagneticLink href={p.href}>{p.link}</MagneticLink></div>{!live&&<img src={p.image} alt={p.title} width="1000" height="769" loading="lazy"/>}</motion.article>)}</div><span className="xp-product-watermark" aria-hidden="true">0{activeProduct+1}</span><div className="xp-collection-bottom"><div className="xp-product-tabs" aria-label="Jump to product">{products.map((p,i)=><a href={live?`#product-${p.id}`:p.href} key={p.id} aria-current={activeProduct===i?'true':undefined}><span>{p.number}</span>{p.title}<ArrowUpRight size={14}/></a>)}</div><span>Three distinct forms.<br/>One considered collection.</span></div></div></section>
    <section id="manufacturing" className="xp-manufacturing"><div className="xp-pattern" aria-hidden="true"/><div className="xp-manufacturing-top"><span className="xp-eyebrow">03 / Made with intent</span><Plus size={30}/></div><Reveal><h2 data-split>Small details.<br/><em>Lasting thinking.</em></h2></Reveal><div className="xp-manufacturing-copy"><p>Better packaging starts long before the first serving. With considered materials, precise manufacturing, and a format that does more with less.</p><Link href="/capabilities">Inside our process <ArrowUpRight size={19}/></Link></div><div className="xp-stats"><div><Count value={11}/><p>Product categories</p></div><div><Count value={3}/><p>Forms. Endless possibilities.</p></div><div><span>PP <Count value={5}/></span><p>Polypropylene options</p></div></div><p className="xp-recycling-note">Material matters. Check local collection and recycling guidance for each format.</p></section>
    <section className="xp-trust"><div className="xp-trust-heading"><span className="xp-eyebrow">At home in your world</span><p>From the corner café to the busiest kitchen.</p></div><div className="xp-marquee" tabIndex={0} aria-label="Packaging for restaurants, cafés, caterers, cloud kitchens, and retail"><div className="xp-marquee-track">{[0,1].map(copy=><div className="xp-marquee-group" aria-hidden={copy===1} key={copy}>{['Restaurants','Cafés & bakeries','Caterers','Cloud kitchens','Food retail'].map((name,i)=><span key={name} className={i%2?'xp-serif':''}>{name}<Plus size={23}/></span>)}</div>)}</div></div></section>
    <section id="closing" className="xp-closing"><Reveal><span className="xp-eyebrow">Your next great idea starts here.</span><h2 data-split>Let’s make<br/><em>something good.</em></h2><MagneticLink href="/contact">Find your packaging</MagneticLink></Reveal>{!live&&<div className="xp-lineup">{products.map(p=><img key={p.id} src={p.image} alt={p.title} loading="lazy" width="1000" height="769"/>)}</div>}<span className="xp-closing-note">Thoughtfully designed. Ready for what’s next.</span></section>
